...
for WORK in ${LIST[@]}
do
	DOCKER_NAME=$(printf "%s%s" "_ubuntupythonrewindprofile" "${WORK}")
	DOCKER_ID=$(docker ps | grep $DOCKER_NAME | awk '{print $1}')
	
	docker cp ${DOCKER_ID}:/tmp/proxy.out ./results/proxy${WORK}.txt 2>> /dev/null
	docker cp ${DOCKER_ID}:/tmp/launcher.out ./results/launcher${WORK}.txt 2>> /dev/null
