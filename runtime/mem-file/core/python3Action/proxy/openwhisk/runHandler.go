...
func (ap *ActionProxy) runHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = json.Unmarshal(response, &tempdata)
	if err != nil {
		sendError(w, http.StatusBadGateway, "DEBUG ERROR 1")
		return
	} else {
		tempdata["runtime_start"] = start.UnixNano()
		tempdata["runtime_end"] = end.UnixNano()
		response, err = json.Marshal(tempdata)
		if err != nil {
			sendError(w, http.StatusBadGateway, "DEBUG ERROR 2")
		}
	}
